...
if __name__ == '__main__':
    ...
    ### 2025-03-10 Jylee
    ### For Tin analysis
    for nfile in range(136):
        filename = '200806_CMD203_Enriched_124Sn.{:03d}.Spc'.format(nfile) #135 files
        try:
            with open('200806_CMD203_Enriched_124Sn.{:03d}.txt'.format(nfile), 'w') as f:
                spectrum = dump_ortec_spc(filename)
                rtime = spectrum['info-acq']['time']['real']
                ltime = spectrum['info-acq']['time']['live']
                f.write(f"{rtime}\n")
                f.write(f"{ltime}\n")
                for k in spectrum['data']['dset']:
                    f.write(f"{k}\n")
        except:
            print('File not found: {}'.format(filename))

    for nfile in range(62):
        filename = '200904_CMD203_Enriched_124Sn.{:03d}.Spc'.format(nfile) #61 files
        try:
            with open('200904_CMD203_Enriched_124Sn.{:03d}.txt'.format(nfile), 'w') as f:
                spectrum = dump_ortec_spc(filename)
                rtime = spectrum['info-acq']['time']['real']
                ltime = spectrum['info-acq']['time']['live']
                f.write(f"{rtime}\n")
    ...
    for nfile in range(265):
        filename = '200918_CMD203_Enriched_124Sn.{:03d}.Spc'.format(nfile) #264 files
        try:
            with open('200918_CMD203_Enriched_124Sn.{:03d}.txt'.format(nfile), 'w') as f:
                spectrum = dump_ortec_spc(filename)
                rtime = spectrum['info-acq']['time']['real']
                ltime = spectrum['info-acq']['time']['live']
                f.write(f"{rtime}\n")
    ...
    for nfile in range(52):
        filename = '201113_CMD203_Enriched_124Sn.{:03d}.Spc'.format(nfile) #51 files
        try:
            with open('201113_CMD203_Enriched_124Sn.{:03d}.txt'.format(nfile), 'w') as f:
                spectrum = dump_ortec_spc(filename)
                rtime = spectrum['info-acq']['time']['real']
                ltime = spectrum['info-acq']['time']['live']
                f.write(f"{rtime}\n")
